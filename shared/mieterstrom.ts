/**
 * shared/mieterstrom.ts
 *
 * Pure Mieterstrom economics: 20-year cashflow model, NPV, IRR (Newton-Raphson)
 * and amortisation. No React, no DOM, no side effects, so both the browser
 * bundle and the server API run exactly the same code.
 */

import type { MieterstromInputs } from "./schema.js";
import { CO2_FACTOR, FEED_IN_TARIFF, MIETERSTROM_ZUSCHLAG, lookupRate } from "./tariffs.js";
...
export const DEFAULTS: MieterstromInputs = {
  ...
  // Banded by plant size — see shared/tariffs.ts.
...
/** Umweltbundesamt figure for 2025 (344 g/kWh). */
